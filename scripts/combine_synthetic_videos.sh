...
ffmpeg \
    ...
    -i "$OVERLAY_BASE" \
    -filter_complex \
"[0:v][2:v]overlay[ours1];\
 [1:v][3:v]overlay[base1];\
 [ours1]scale=600x400,setpts=PTS-STARTPTS[base];\
 [base1]scale=600x400,setpts=PTS-STARTPTS[overlay];\
 color=0x00000000:s=600x400:d=$LEN,format=rgba[black];\
 color=0xffffffff:s=600x400:d=$LEN,format=rgba[white];\
